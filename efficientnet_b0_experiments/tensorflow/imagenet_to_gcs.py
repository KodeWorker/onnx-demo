...
flags.DEFINE_string(
    'file_format', 'PNG', 'format of image files: png, jpg.')
flags.DEFINE_string(
...
def convert_to_tf_records(raw_data_dir):
  ...
  training_files = tf.gfile.Glob(
      os.path.join(raw_data_dir, TRAINING_DIRECTORY, '*', '*.' + FLAGS.file_format))
  ...
  validation_files = sorted(tf.gfile.Glob(
      os.path.join(raw_data_dir, VALIDATION_DIRECTORY, '*.' + FLAGS.file_format)))
...
def convert_to_tf_records_general(raw_data_dir):
  random.seed(0)
  ...
  raw_date_splits = os.listdir(raw_data_dir)

  training_files = tf.gfile.Glob(
      os.path.join(raw_data_dir, TRAINING_DIRECTORY, '*', '*.PNG'))
